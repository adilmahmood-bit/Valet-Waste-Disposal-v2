...
const featureCards = [
  {
    ...
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#52b548" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    icon: (
      <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#52b548" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round">
        ...
        <path d="M8.5 14.5l1.5-3.5h4l1.5 3.5-3.5 4.5-3.5-4.5z" />
      </svg>
    ),
  },
  {
        ...
        <RevealSection>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 max-w-2xl mx-auto">
            {featureCards.map((card) => (
              <div
                key={card.label}
                className="bg-white rounded-xl px-3 py-4 flex flex-col items-center gap-2 shadow-md text-center transition-transform duration-200 hover:-translate-y-1.5 cursor-default"
              >
                {card.icon}
                <span className="text-sm font-bold text-gray-800 leading-snug">{card.label}</span>
              </div>
            ))}
          </div>
        </RevealSection>
